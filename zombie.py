...
class Zombie:
    ...
    def draw(self):
        Zombie.marker_image.draw(self.tx-10, self.ty - 10) #화살표를 먼저그린다
    ...
    def move_slightly_to_opposite(self, tx, ty):
        self.dir = -math.atan2(ty - self.y, tx - self.x)
        distance = RUN_SPEED_PPS * game_framework.frame_time
        self.x -= distance * math.cos(self.dir)
        self.y += distance * math.sin(self.dir)
    ...
    def run_from_boy(self):
        self.state = 'Walk'
        self.move_slightly_to_opposite(play_mode.boy.x, play_mode.boy.y)
    ...
    def build_behavior_tree(self):
        ...
        move_to_target_loaction = Sequence('Move to target location', a1, a2)

        a3 = Action('Set random location', self.set_random_location)
        wander = Sequence('Wander', a3, a2)

        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        c2 = Condition('소년이 더 센가?', self.is_boy_stronger)
        c3 = Condition('좀비가 더 센가?', self.is_zombie_stronger)

        a4 = Action('소년한테 접근', self.move_to_boy)
        chase_boy = Sequence('소년을 추적', c1, c3, a4)

        a5 = Action('소년에게서 도망', self.run_from_boy)
        run_away = Sequence('소년을 피해 도망', c1, c2, a5)

        chase_or_run = Selector('추적 또는 도망', chase_boy, run_away)
        root = do_or_wander = Selector('행동 또는 배회', chase_or_run, wander)

        #root = chase_or_flase = Selector('추적 또는 배회', chase_boy, wander)
